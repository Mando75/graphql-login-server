...
export async function studentLogin(data) {
  return await StudentModel.findOne({unit_id: data.unit_id}, '_id first_name last_name section type', (err, user) => {
    if (err) {
...
export async function findStudentAuth(student_id) {
  return await StudentModel.findById(student_id, '_id first_name last_name section type', (err, user) => {
    if (err) {
